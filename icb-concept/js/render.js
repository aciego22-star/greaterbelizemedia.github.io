...
(function () {
  ...
  /* Everything user-visible goes through here, so this is the right place
     to resolve a translated value. A data field may be a plain string or
     an { en, es } pair, and a view should not have to know which: passing
     the pair straight in used to render "[object Object]". */
  function esc(s) {
    if (s != null && typeof s === "object" && !Array.isArray(s) &&
        (typeof s.en === "string" || typeof s.es === "string")) {
      s = ICB.i18n.t(s);
    }
  ...
  function extNote(host) {
    return '<span class="visually-hidden"> (' + esc(ICB.s("openInNewTab", { host: host || "external site" })) + ")</span>";
  }
  ...
  function productCard(p) {
    var first = p.suspended
      ? '<a href="' + esc(p.route) + '">' + esc(ICB.s("viewCurrentInfo")) + "</a>"
      : '<a href="' + esc(p.route) + '">' + esc(ICB.s("learnMore")) + "</a>";
    var second;
    if (p.suspended) {
      second = '<a href="#/contact?topic=other&category=' + esc(p.id) + '">' + esc(ICB.s("contactICB")) + "</a>";
    } else if (p.quote === true) {
      second = '<a href="' + esc(ICB.DATA.quoteHref(p.id)) + '">' + esc(ICB.s("requestQuote")) + "</a>";
    } else {
      second = '<a href="#/contact?topic=other&category=' + esc(p.id) + '">' + esc(ICB.s("contactICB")) + "</a>";
    }
    return '' +
      '<article class="card rv" data-product-card="' + esc(p.id) + '">' +
        ...
        '<div class="card-body">' +
          ...
          '<p class="card-desc">' + esc(ICB.t(p.short)) + "</p>" +
  ...
  function actionTile(t) {
    ...
    out += '<span class="tile-label">' + esc(ICB.t(t.label)) + (external ? extNote(hostOf(t.href)) : "") + "</span>";
    if (t.note) out += '<span class="tile-note">' + esc(ICB.t(t.note)) + (external ? ' <span aria-hidden="true">&#8599;</span>' : "") + "</span>";
    return out + "</a>";
  }
    ...
    return '' +
      '<article class="pathway rv" id="claim-' + esc(c.anchor) + '" data-anchor="' + esc(c.anchor) + '">' +
        '<div class="pathway-head">' +
          ...
          "<h3>" + esc(ICB.t(c.name)) + "</h3>" +
        "</div>" +
        '<p class="pathway-lead">' + esc(ICB.t(c.lead)) + "</p>" +
        '<div class="pathway-actions">' +
          ...
          '<a class="btn btn-ghost btn-sm" href="#/contact?topic=claim">' + esc(ICB.s("contactICB")) + "</a>" +
        "</div>" +
        '<p class="pathway-note">' + esc(ICB.s("claimsFormNote")) + "</p>" +
      "</article>";
  }
  ...
  function locationCard(l) {
    ...
    out += '<div class="loc-head"><h3>' + esc(l.name) + "</h3>" +
      '<span class="badge' + (l.type === "Agency" ? " badge--gold" : "") + '">' + esc(ICB.s("locType" + l.type.replace(/\s+/g, ""))) + "</span></div>";
    ...
    for (var i = 0; i < l.phones.length; i++) {
      out += '<a class="msg-action" href="tel:' + esc(l.phones[i].tel) + '">' + ICB.art.glyph("phone") +
        "<span>" + esc(ICB.s("callDisplay", { n: l.phones[i].display })) + "</span></a>";
    }
    ...
    callable.forEach(function (w) {
      ...
      out += '<a class="msg-action" href="tel:+' + esc(digits) + '">' + ICB.art.glyph("phone") +
        "<span>" + esc(ICB.s("callDisplay", { n: w.display })) + "</span></a>";
    });
    ...
    if (!l.phones.length && !callable.length && l.corporateLine) {
      ...
      out += '<a class="msg-action msg-action--corp" href="tel:' + esc(co.phoneTel) + '">' + ICB.art.glyph("phone") +
        "<span>" + esc(ICB.s("corporateOfficeN", { n: co.phoneDisplay })) + "</span></a>";
    }
    ...
    out += '<a class="msg-action" href="https://www.google.com/maps/search/?api=1&query=' + encodeURIComponent(l.mapQuery) + '"' + extAttrs() + ">" +
      ICB.art.glyph("marker") + "<span>" + esc(ICB.s("directions")) + "</span>" + extNote("google.com/maps") + "</a>";
    out += "</div></article>";
    return out;
  }
  ...
  function steps(list) {
    return '<ol class="steps">' + list.map(function (s) {
      return '<li class="step rv"><span class="step-num" aria-hidden="true">' + s.n + "</span>" +
        "<div><h3>" + esc(ICB.t(s.title)) + "</h3><p>" + esc(ICB.t(s.body)) + "</p></div></li>";
    }).join("") + "</ol>";
  }
  ...
  function crumbs(trail) {
    ...
    return '<nav class="crumbs crumbs--dark" aria-label="' + esc(ICB.s("breadcrumb")) + '"><ol>' + items + "</ol></nav>";
  }

  /* An interior page one level below the homepage. */
  function crumbsHome(label) {
    return crumbs([{ label: ICB.s("home"), href: "#/" }, { label: label }]);
  }

  /* Section heading block. */
  function sectionHead(opts) {
    var out = '<div class="section-head' + (opts.center ? " section-head--center" : "") + (opts.rv === false ? "" : " rv") + '">';
    if (opts.eyebrow) out += '<span class="eyebrow">' + esc(ICB.t(opts.eyebrow)) + "</span>";
    out += "<h2>" + esc(ICB.t(opts.title)) + "</h2>";
    if (opts.sub) out += "<p>" + esc(ICB.t(opts.sub)) + "</p>";
  ...
  function band(opts) {
    ...
    if (opts.eyebrow) out += '<span class="eyebrow">' + esc(ICB.t(opts.eyebrow)) + "</span>";
    out += "<" + (opts.hTag || "h2") + ">" + esc(ICB.t(opts.title)) + "</" + (opts.hTag || "h2") + ">";
    if (opts.body) out += "<p>" + esc(ICB.t(opts.body)) + "</p>";
  ...
  var QUIZ_OPTIONS = [
    { id: "home", label: { en: "My home", es: "Mi casa" }, glyph: "house", productId: "property",
      blurb: { en: "Homeowners and renters are both part of ICB's published property categories.", es: "Home Owners y Renters forman parte de las categorías de propiedad que publica ICB." } },
    { id: "vehicle", label: { en: "My vehicle", es: "Mi vehículo" }, glyph: "car", productId: "motor",
      blurb: { en: "From personal vehicles to taxis, buses and heavy-duty equipment.", es: "Desde vehículos personales hasta taxis, autobuses y equipo pesado." } },
    { id: "business", label: { en: "My business", es: "Mi negocio" }, glyph: "briefcase", special: "business",
      blurb: { en: "ICB publishes insurance categories for businesses across Belize.", es: "ICB publica categorías de seguros para empresas en todo Belice." } },
    { id: "boat", label: { en: "A boat or vessel", es: "Una embarcación" }, glyph: "boat", productId: "marine",
      blurb: { en: "Marine Hull Insurance, customizable to third party and passenger liability.", es: "Marine Hull Insurance, personalizable para responsabilidad ante terceros y pasajeros." } },
    { id: "goods", label: { en: "Goods in transit", es: "Mercancía en tránsito" }, glyph: "container", productId: "cargo",
      blurb: { en: "Warehouse-to-warehouse protection for goods in transit.", es: "Protección de bodega a bodega para mercancía en tránsito." } },
    { id: "commercial-liability", label: { en: "Commercial liability", es: "Responsabilidad comercial" }, glyph: "scales", productId: "liability",
      blurb: { en: "Liability and miscellaneous products arranged around specific business exposures.", es: "Productos de responsabilidad y misceláneos organizados según exposiciones específicas del negocio." } },
    /* Sales are suspended, so this result says so rather than presenting
       Travel Insurance as something to arrange. */
    { id: "trip", label: { en: "My next trip", es: "Mi próximo viaje" }, glyph: "plane", productId: "travel",
      blurb: { en: "Sales of ICB Travel Insurance are currently temporarily suspended.", es: "La venta de Travel Insurance de ICB está temporalmente suspendida." } },
    /* No legal assertions about driving in Mexico: ICB presents this line
       entirely through ANA Seguros. */
    { id: "mexico", label: { en: "A drive into Mexico", es: "Un viaje a México" }, glyph: "border", productId: "mexican",
      blurb: { en: "ICB's Mexican Insurance is provided through ANA Seguros.", es: "Mexican Insurance de ICB se ofrece a través de ANA Seguros." } },
    { id: "unsure", label: { en: "I'm not sure", es: "No estoy seguro" }, glyph: "question", special: "unsure",
      blurb: { en: "No problem. The ICB team will point you in the right direction.", es: "No hay problema. El equipo de ICB le orientará." } }
  ];

  /* The note under a quote action, so a result never reads as a price. */
  function quoteNote() {
    return '<p class="quiz-note">' + esc(ICB.t(ICB.DATA.quoteNote)) + "</p>";
  }

  function quizResult(opt) {
    var out = '<span class="eyebrow">' + esc(ICB.s("quizStart")) + "</span>";
    if (opt.special === "business") {
      out += "<h3>" + esc(ICB.s("quizBusinessTitle")) + "</h3><p>" + esc(ICB.t(opt.blurb)) + "</p>" +
        '<div class="btn-row">' +
        '<a class="btn btn-primary" href="#/business">' + esc(ICB.s("exploreBusiness")) + "</a>" +
        '<a class="btn btn-ghost" href="#/contact?topic=business">' + esc(ICB.s("requestQuote")) + "</a>" +
        "</div>" + quoteNote();
    } else if (opt.special === "unsure") {
      out += "<h3>" + esc(ICB.s("quizUnsureTitle")) + "</h3><p>" + esc(ICB.t(opt.blurb)) +
        " " + esc(ICB.s("quizUnsureExtra")) + "</p>" +
        '<div class="btn-row">' +
        '<a class="btn btn-primary" href="#/contact?topic=new-cover">' + esc(ICB.s("talkToICB")) + "</a>" +
      ...
      if (p.suspended) {
        return '<span class="eyebrow">' + esc(ICB.s("currentStatus")) + "</span>" +
          "<h3>" + esc(p.name) + "</h3><p>" + esc(ICB.t(opt.blurb)) + "</p>" +
          '<div class="btn-row">' +
          '<a class="btn btn-primary" href="' + esc(p.route) + '">' + esc(ICB.s("viewCurrentInfo")) + "</a>" +
          '<a class="btn btn-ghost" href="#/contact?topic=other&category=' + esc(p.id) + '">' + esc(ICB.s("contactICB")) + "</a>" +
          "</div>";
      }
      ...
      var second = p.quote === true
        ? '<a class="btn btn-primary" href="' + esc(ICB.DATA.quoteHref(p.id)) + '">' + esc(ICB.s("requestQuote")) + "</a>"
        : '<a class="btn btn-primary" href="#/contact?topic=other&category=' + esc(p.id) + '">' + esc(ICB.s("contactICB")) + "</a>";
      out += "<h3>" + esc(p.name) + "</h3><p>" + esc(ICB.t(opt.blurb)) + "</p>" +
        '<div class="btn-row">' +
        second +
        '<a class="btn btn-ghost" href="' + esc(p.route) + '">' + esc(ICB.s("learnAboutProduct", { product: p.name })) + "</a>" +
        "</div>" +
  ...
  function quiz(headingId) {
    var opts = QUIZ_OPTIONS.map(function (o) {
      return '<button type="button" class="quiz-option" data-quiz-option="' + o.id + '" aria-pressed="false">' +
        ...
        '<span class="tile-label">' + esc(ICB.t(o.label)) + "</span></button>";
    }).join("");
    return '<div class="quiz-panel rv">' +
      '<h2 id="' + esc(headingId || "quiz-title") + '">' + esc(ICB.s("quizTitle")) + "</h2>" +
      '<div class="quiz-grid" role="group" aria-label="' + esc(ICB.s("quizGroupLabel")) + '">' + opts + "</div>" +
  ...
  function branchTile(b, photoIndex) {
    ...
    var cap = '<figcaption>' +
        '<span class="g-name">' + esc(ICB.t(b.caption)) + "</span>" +
    ...
    return '<figure class="' + cls + ' gallery-item--plate">' +
      '<a class="g-open g-plate" href="#/locations" aria-label="' + esc(ICB.s("seeOnLocations", { name: ICB.t(b.caption) })) + '">' +
        '<span class="g-plate-inner">' +
          '<span class="g-plate-mark">' + ICB.art.glyph("marker") + "</span>" +
          '<span class="g-plate-name">' + esc(ICB.t(b.caption)) + "</span>" +
      ...
      '<figure class="film rv" data-film lang="' + esc(f.lang) + '">' +
        '<div class="film-frame">' +
          ...
          '<button type="button" class="play-btn" data-film-play' +
            ' aria-label="' + esc(ICB.s("playFilm", { title: f.altTitle || f.title })) + '">' +
            ICB.art.glyph("play") +
          "</button>" +
          '<p class="video-note" data-film-note hidden>' + esc(ICB.s("filmUnavailable")) + "</p>" +
          '<span class="film-lang" aria-hidden="true">' + esc(f.langLabel) + "</span>" +
        "</div>" +
        '<figcaption class="film-cap">' +
          '<span class="eyebrow">' + esc(ICB.t(f.kicker)) + "</span>" +
          "<h3>" + esc(f.title) + "</h3>" +
          "<p>" + esc(ICB.t(f.blurb)) + "</p>" +
        "</figcaption>" +
      "</figure>";
  }
  ...
  function motionSection(opts) {
    ...
    return '' +
      '<section class="section' + (opts.tint ? " section--tint" : "") + '" aria-labelledby="motion-title">' +
        '<div class="shell">' +
          sectionHead({
            eyebrow: ICB.s("icbInMotion"),
            title: opts.title || ICB.s("theICBFilms"),
            sub: opts.sub || null,
            id: "motion-title"
          }) +
